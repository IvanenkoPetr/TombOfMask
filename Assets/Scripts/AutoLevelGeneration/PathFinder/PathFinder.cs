...
namespace AStar
{
    public class PathFinder : IPathFinder
    {
        ...
        public static bool IsLevelPassable()
        {
            ...
            for(var i = 0; i < levelStructure.GetLength(0); i++)
            {
                for (var j = 0; j < levelStructure.GetLength(1); j++)
                {
                    ...
                    pathGrid[i, j] = (byte)(tile.IsPassable ? 1 : 0);
        ...
        public void ModifyGridItem(Point position, bool passable)
        {
            _grid[position.X, position.Y] = (byte)(passable ? 1 : 0);
        }

        public bool IsGridItemPassable(Point position)
        {
            return _grid[position.X, position.Y] == 1;
        }

        public bool IsValidMapPoint(Point pt)
        {
            if (pt.X < 0 ||
                pt.Y < 0 ||
                pt.X >= _grid.GetLength(0) ||
                pt.Y >= _grid.GetLength(1)
                ) return false;

            return true;
    }

        public List<PathFinderNode> FindPath(Point start, Point end, int maxSteps = 1000)
        {
            _options.SearchLimit = maxSteps;
